...
if [ $# -eq 1 ]; then
  ...
  psql -c "" -h 127.0.0.1 -U "$POSTGRES_ADMIN" postgres || print_error "Current user is not an administrator for PostgreSQL or has a password. Remove admin password (if any) and run the script again with 'sudo -u admin ...'"
